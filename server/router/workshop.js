...
router.get('/viewappointment/:id',authenticate,async (req,res)=>{
    const {id} = req.params;
    try{
    const workshop=await Workshop.findById(id);
    const appointment=await Appointment.find({name:workshop.name}).populate('user');
    if(appointment){
        res.status(200).json(appointment);
    }
    else{
        res.status(422).json({error:"No appointments"})
    }
    }catch(err){
        console.log(err);
    }
